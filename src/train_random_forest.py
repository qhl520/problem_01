...
def main() -> None:
    ...
    purchase_model = train_model("random_forest", X, train_feat["purchase"])
    redeem_model = train_model("random_forest", X, train_feat["redeem"])
    ...
    result.to_csv(OUTPUT_DIR / "validation_random_forest.csv", index=False)
    save_model(purchase_model, MODEL_DIR / "random_forest_purchase.pkl")
    save_model(redeem_model, MODEL_DIR / "random_forest_redeem.pkl")
    save_feature_list(feature_cols, MODEL_DIR / "random_forest_features.json")
    feature_importance_frame(purchase_model, feature_cols).to_csv(OUTPUT_DIR / "feature_importance_rf_purchase.csv", index=False)
    feature_importance_frame(redeem_model, feature_cols).to_csv(OUTPUT_DIR / "feature_importance_rf_redeem.csv", index=False)
    append_experiment(
